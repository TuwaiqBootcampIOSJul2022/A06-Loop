...
for (index, characte) in alphabet.enumerated() {
    if index % 2 == 0 {
        print("This is a \(characte)")
    }
}
...
let namesOfStates = ["Riyadh":"KSA", "Abu Dhabi":"UAE","Amman":"Jordan"]
for (Key, Value) in namesOfStates {
    print("\(Value) the capital of \(Key)")
    if Key == "Riyadh" {
