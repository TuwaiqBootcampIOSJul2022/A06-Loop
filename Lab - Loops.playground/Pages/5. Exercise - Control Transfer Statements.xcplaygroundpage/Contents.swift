...
for (Keys,Values) in alphabet.enumerated(){
    if Keys.isMultiple(of:2){
        print("\(Values)")
    }
}
...
var Home = "Riyadh"
var dictionary:[String:String] = ["Damam":"Riyadh","Hail":"Jeddah","Alkarij":"Arar"]
for (Keys , Values) in dictionary{
    if Keys == Home || Values == Home {
